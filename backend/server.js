    ...
    if (now - bridge.lastActivity > bridgeTimeout) {
      ...
      try {
        bridge.clients.forEach((client) => {
          ...
          try {
            client.primaryConnection.destroy();
          } catch {}
...
app.post("/api/tcp", async (req, res) => {
  ...
  if (!ip || !port || !Array.isArray(data_list) || data_list.length === 0) {
    return res
      .status(400)
      .json({ error: "ip, port, and non-empty data_list are required" });
  }
      ...
      timer = setTimeout(
        () => {
          if (pendingResolve && !isResponseHandled) {
            addEvent(
              "timeout",
              `Timeout waiting for response to packet #${currentPacketIndex + 1}`,
            );
  ...
  sock.on("data", (chunk) => {
    ...
    addEvent("data", `Received ${chunk.length} bytes`, {
      hex,
      ascii,
      length: chunk.length,
    });
    ...
    if (currentPacketIndex >= 0) {
      ...
      results[currentPacketIndex].responses.push({
        hex,
        ascii,
        length: chunk.length,
      });
  ...
  sock.connect(Number(port), String(ip), async () => {
    ...
    try {
      for (let i = 0; i < data_list.length; i++) {
        ...
        addEvent("send", `Sent packet #${i + 1} (${buf.length} bytes)`, {
          hex: sentHex,
          ascii: sentAscii,
        });
    ...
    if (!responseSent) {
      ...
      res.json({
        ...
        summary: {
          ...
          totalResponsesReceived: results.reduce(
            (sum, r) => sum + (r.responses?.length || 0),
            0,
          ),
        },
      });
...
app.post("/api/tcp/send", async (req, res) => {
  ...
  if (!conn) {
    return res.status(404).json({
      error: "Connection not found. It may have timed out or been closed.",
    });
  }
  ...
  try {
    ...
    try {
      ...
      return res.status(400).json({
        error: `Failed to parse data as ${data_format}: ${parseErr.message}`,
      });
    }
    ...
    addEvent("send", `Sent ${buf.length} bytes`, {
      hex: sentHex,
      ascii: sentAscii,
    });
      ...
      await new Promise((resolve, reject) => {
        socket.send(buf, 0, buf.length, Number(port), String(ip), (err) => {
          if (err) {
            addEvent(
              "error",
              `Failed to send packet #${i + 1}: ${err.message}`,
            );
            results.push({
              packetIndex: i,
              sent: pkt,
            ...
            addEvent(
              "send",
              `Sent packet #${i + 1} (${buf.length} bytes) to ${ip}:${port}`,
              { hex: sentHex, ascii: sentAscii },
            );
          ...
          const checkInterval = setInterval(() => {
            if (responseReceived || Date.now() - startTime > timeout * 1000) {
              ...
              if (!responseReceived) {
                addEvent(
                  "timeout",
                  `No response received for packet #${i + 1} within ${timeout}s`,
                );
              }
              resolve();
            }
...
app.post("/api/mqtt/connect", async (req, res) => {
  ...
  try {
    ...
    client.on("connect", () => {
      addEvent("connect", `Connected to ${brokerUrl}`, {
...
app.post("/api/mqtt/publish", async (req, res) => {
  ...
  if (!sessionId || !topic || message === undefined) {
    return res
      .status(400)
      .json({ error: "sessionId, topic, and message are required" });
  }

  const conn = mqttConnectionPool.get(sessionId);
  if (!conn) {
    return res.status(404).json({
      error: "MQTT connection not found. It may have timed out or been closed.",
    });
  }
    ...
    await new Promise((resolve, reject) => {
      conn.client.subscribe(topic, { qos }, (err, granted) => {
        if (err) {
          ...
          addEvent("subscribe", `Subscribed to topic: ${topic}`, {
            topic,
            qos,
            granted,
          });
    ...
    await new Promise((resolve, reject) => {
      conn.client.unsubscribe(topic, (err) => {
        if (err) {
          ...
          addEvent("unsubscribe", `Unsubscribed from topic: ${topic}`, {
            topic,
          });
          resolve();
        }
...
// ---- /api/load-est/http-request: Proxy HTTP requests to external APIs ----
app.post("/api/http-request/load-test/start", async (req, res) => {
  try {
    if (k6Process?.length) {
    ...
    const { imeis, bodyTemplate, durationMinutes = 5, simulation } = req.body;

    if (!imeis?.length) {
      return res.status(400).json({
        success: false,
        message: "IMEIs are required",
      });
    }

    const duration = Math.min(Math.max(Number(durationMinutes) || 5, 1), 15);
    const scriptPath = path.join(__dirname, "./K6-LoadTesting/k6-runner.js");
    const k6Path = path.join(__dirname, "./K6-LoadTesting/k6/k6.exe");
    ...
    if (!io) {
      ...
      io.on("connection", (socket) => {
        ...
        loadTestSocket = socket;

        socket.on("disconnect", () => {
          console.log("Frontend disconnected:", socket.id);

          if (loadTestSocket?.id === socket.id) {
            loadTestSocket = null;
          }
        });
      });
    ...
    k6Process = spawn(k6Path, ["run", scriptPath], {
      env: {
        ...process.env,

        IMEIS: JSON.stringify(imeis),
        BODY_TEMPLATE: JSON.stringify(bodyTemplate),
        SIMULATION: JSON.stringify(simulation),

        CPU_COUNT: String(cpuCount),
        DURATION: `${duration}m`,
      },
    ...
    k6Process.stdout.on("data", (data) => {
      ...
      console.log(`[k6] ${message}`);

      io?.emit("loadtest:log", {
        level: "info",
        message,
        timestamp: Date.now(),
      });
    ...
    k6Process.stderr.on("data", (data) => {
      ...
      console.error(`[k6 ERROR] ${message}`);
    ...
    // -----------------------------------------
    // k6 completed / stopped / crashed
    // -----------------------------------------

    k6Process.on("close", async (code, signal) => {
      console.log(`k6 exited. code=${code}, signal=${signal}`);

      const testEndTime = Date.now();
      const actualRuntime = (testEndTime - testStartTime) / 1000;
      let k6Metrics = null;

      try {
        const summary = await fs.readFile("./k6-summary.json", "utf8");
        k6Metrics = JSON.parse(summary);
      } catch (error) {
        console.error("Unable to read k6 summary:", error);
      }

      const status = signal ? "stopped" : code !== 0 ? "failed" : "completed";
      const metrics = k6Metrics?.metrics;
      const finalStats = {
        total: metrics?.http_reqs?.values?.count ?? 0,
        rps: metrics?.http_reqs?.values?.rate ?? 0,
        success:
          (metrics?.http_reqs?.values?.count ?? 0) -
          (metrics?.http_req_failed?.values?.passes ?? 0),
        failed: metrics?.http_req_failed?.values?.passes ?? 0,
        avgLatency: metrics?.http_req_duration?.values?.avg ?? 0,
        testDuration: duration * 60,
        actualRuntime,
        drainTime: Math.max(actualRuntime - duration * 60, 0),
        targetRps: imeis?.length,
        actualRps: metrics?.http_reqs?.values?.rate ?? 0,
...
app.post("/api/http-request/load-test/stop", (req, res) => {
  if (!k6Process?.length) {
    return res.json({
      success: false,
      message: "No running load test",
    });
  }

  console.log("Stopping k6 load test...");

  k6Process.kill("SIGINT");

  return res.json({
    success: true,
    message: "Load test stopping",
  });
});
...
app.post("/api/fcm", async (req, res) => {
  try {
    const { projectId, token, notification, apns, data, serviceAccount } =
      req.body || {};
    if (!projectId || !token)
      return res
        .status(400)
        .json({ error: "projectId and token are required" });
...
app.post("/api/tcp-bridge/start", async (req, res) => {
  ...
  if (!listenPort || !primaryServer?.ip || !primaryServer?.port) {
    return res.status(400).json({
      error:
        "listenPort, primaryServer.ip, and primaryServer.port are required",
    });
  }
  ...
  if (
    isLocalhost(primaryServer.ip) &&
    Number(primaryServer.port) === Number(listenPort)
  ) {
    return res.status(400).json({
  ...
  for (let i = 0; i < secondaryServers.length; i++) {
    ...
    if (server.ip && server.port) {
      if (
        isLocalhost(server.ip) &&
        Number(server.port) === Number(listenPort)
      ) {
        return res.status(400).json({
  ...
  const allServers = [
    ...
    ...secondaryServers.map((s, idx) => ({
      ...s,
      type: `secondary-${idx + 1}`,
    })),
  ];
  ...
  try {
    const server = net.createServer((clientSocket) => {
      ...
      addLog("client_connected", `Client connected: ${clientInfo}`, {
        clientId,
        clientInfo,
      });
      ...
      primaryConn.connect(primaryServer.port, primaryServer.ip, () => {
        addLog(
          "primary_connected",
          `Connected to primary server ${primaryServer.ip}:${primaryServer.port}`,
          { clientId },
        );
      });
      ...
      const secondaryConns = secondaryServers.map((server, idx) => {
        ...
        conn.connect(server.port, server.ip, () => {
          addLog(
            "secondary_connected",
            `[Secondary ${idx + 1}] Connected to ${serverInfo}`,
            {
              clientId,
              serverIndex: idx,
              serverInfo,
        ...
        conn.on("data", (data) => {
          ...
          addLog(
            "secondary_data",
            `[Secondary ${idx + 1} ➜ Bridge] Received ${data.length} bytes from ${serverInfo} (NOT forwarded to client)`,
            {
              clientId,
        ...
        conn.on("error", (err) => {
          addLog(
            "secondary_error",
            `[Secondary ${idx + 1}] Error from ${serverInfo}: ${err.message}`,
            {
              clientId,
              serverIndex: idx,
              serverInfo,
        ...
        conn.on("close", () => {
          addLog(
            "secondary_closed",
            `[Secondary ${idx + 1}] Connection closed to ${serverInfo}`,
            {
              clientId,
              serverIndex: idx,
              serverInfo,
      ...
      clientSocket.on("data", (data) => {
        ...
        const destinations = [
          `Primary (${primaryServer.ip}:${primaryServer.port})`,
        ];
        ...
        try {
          ...
          addLog(
            "forward_primary_error",
            `[Bridge ✖ Primary] Failed to forward: ${err.message}`,
            {
              clientId,
        ...
        secondaryConns.forEach((conn, idx) => {
          try {
            ...
            addLog(
              "forward_secondary_error",
              `[Bridge ✖ Secondary ${idx + 1}] Failed: ${err.message}`,
              {
                clientId,
        ...
        if (primarySuccess || secondarySuccesses.length > 0) {
          const destCount =
            (primarySuccess ? 1 : 0) + secondarySuccesses.length;
          const destText = destinations.join(", ");
          addLog(
            "client_forward_all",
            `[Client ➜ Bridge ➜ ${destCount} Server${destCount > 1 ? "s" : ""}] Received ${data.length} bytes from ${clientInfo} and forwarded to: ${destText}`,
            {
              clientId,
              clientInfo,
      ...
      primaryConn.on("data", (data) => {
        ...
        try {
          ...
          addLog(
            "primary_response",
            `[Primary ➜ Bridge ➜ Client] Received ${data.length} bytes from ${primaryServer.ip}:${primaryServer.port} and forwarded to ${clientInfo}`,
            {
              clientId,
              clientInfo,
          ...
          addLog(
            ...
            `[Bridge ✖ Client] Failed to forward: ${err.message}`,
            {
              clientId,
              clientInfo,
      ...
      clientSocket.on("close", () => {
        addLog("client_disconnected", `Client disconnected: ${clientInfo}`, {
          clientId,
        });
      ...
      clientSocket.on("error", (err) => {
        addLog("client_error", `Client error: ${err.message}`, {
          clientId,
          clientInfo,
        });
      });
      ...
      primaryConn.on("error", (err) => {
        addLog("primary_error", `Primary server error: ${err.message}`, {
          clientId,
        });
        try {
          clientSocket.end();
      ...
      primaryConn.on("close", () => {
        addLog("primary_closed", `Primary server connection closed`, {
          clientId,
        });
        try {
          clientSocket.end();
...
app.get("/api/tcp-bridge/status/:bridgeId", (req, res) => {
  ...
  const clientsInfo = Array.from(bridge.clients.entries()).map(
    ([id, client]) => ({
      clientId: id,
...
app.post("/api/http-bridge/start", async (req, res) => {
  ...
  if (!listenPort || !primaryUrl) {
    return res
      .status(400)
      .json({ error: "listenPort and primaryUrl are required" });
  }
  ...
  try {
    ...
    return res
      .status(400)
      .json({ error: `Invalid primary URL: ${primaryUrl}` });
  }
  ...
  const primaryEffectivePort = parsedPrimary.port
    ...
    : parsedPrimary.protocol === "https:"
      ? 443
      : 80;

  if (
    isLocalhost(parsedPrimary.hostname) &&
    primaryEffectivePort === Number(listenPort)
  ) {
    return res.status(400).json({
  ...
  for (let i = 0; i < secondaryUrls.length; i++) {
    ...
    try {
      ...
      return res
        .status(400)
        .json({ error: `Invalid secondary URL ${i + 1}: ${secondaryUrls[i]}` });
    }
    const secPort = parsedSec.port
      ? Number(parsedSec.port)
      : parsedSec.protocol === "https:"
        ? 443
        : 80;
  ...
  const portInUse = [...httpBridgePool.values()].some(
    (b) => String(b.port) === String(listenPort),
  );
  if (portInUse) {
    return res.status(400).json({
      error: `Port ${listenPort} is already in use by another HTTP bridge.`,
    });
  }
  ...
  const httpServer = http.createServer(async (clientReq, clientRes) => {
    ...
    addLog(
      "request",
      `${clientReq.method} ${reqPath} from ${clientReq.socket?.remoteAddress}`,
      {
        requestId,
    ...
    const noBody = ["GET", "HEAD", "OPTIONS"].includes(
      (clientReq.method || "").toUpperCase(),
    );
    ...
    fetch(targetUrl, {
      ...
      .then(async (primaryResponse) => {
        ...
        addLog(
          "primary_response",
          `Primary → ${primaryResponse.status} in ${Date.now() - startTime}ms`,
          {
            requestId,
      ...
      .catch((err) => {
        const errMsg =
          err.name === "TimeoutError" || err.name === "AbortError"
            ? "Primary server timed out"
            : err.message;
        addLog("error", `Primary failed: ${errMsg}`, {
          requestId,
          error: errMsg,
          duration: Date.now() - startTime,
        });
    ...
    baseSecondaryUrls.forEach((secBase, i) => {
      ...
      fetch(secUrl, {
        ...
        .then(async (secRes) => {
          ...
          addLog(
            "secondary_response",
            `Secondary ${i + 1} → ${secRes.status} in ${Date.now() - secStart}ms`,
            {
              requestId,
              secondaryIndex: i + 1,
              secondaryUrl: secUrl,
        ...
        .catch((err) => {
          addLog(
            "secondary_error",
            `Secondary ${i + 1} failed: ${err.message}`,
            {
              requestId,
              secondaryIndex: i + 1,
              secondaryUrl: secUrl,
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: `Failed to start HTTP bridge: ${err.message}` });
  }
...
app.post("/api/http-bridge/stop", async (req, res) => {
  ...
  if (!bridge) {
    return res
      .status(404)
      .json({ error: "HTTP Bridge not found or already stopped" });
  }
